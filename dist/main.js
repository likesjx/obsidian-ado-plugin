...
var SettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(this.containerEl).setName("Refresh Interval").setDesc("Set the interval (in minutes) for refreshing data from Azure DevOps.").addText((text) => text.setPlaceholder("Enter interval in minutes").setValue(refreshInterval.toString()).onChange(async (value) => {
...
// src/main.ts
var ADOPlugin = class extends import_obsidian2.Plugin {
  async onload() {
...
var EpicNumberModal = class extends import_obsidian2.Modal {
  ...
  submit(value) {
    if (!/^[0-9]+$/.test(value.trim())) {
      new import_obsidian2.Notice("Please enter a valid number.");
      return;
    }
    this.close();
...
function insertEpicAnchor(app, editor) {
  const selection = editor.getSelection();
  if (/^[0-9]+$/.test(selection)) {
    editor.replaceSelection(`<<#${selection}>>`);
    return;
  }
  new EpicNumberModal(app, (epicNumber) => {
